...
class TestMakeTransaction:
    ...
    def test_source_customer_end_balance(self, sender, receiver, amount, result):
    ...
    def test_target_customer_end_balance(self, sender, receiver, amount, result):
        sender.make_transaction(receiver, amount=amount)
        assert result == receiver.balance
        
    # edge case - transact just enough money
    def test_sends_all_balance(self, sender, receiver):
        sender.make_transaction(receiver, amount=100)
        assert 200 == receiver.balance
        
    # bad case - transact negative money
    def test_negative_value_transaction_raises_errror(self, sender, receiver):
        with pytest.raises(ValueError):
            sender.make_transaction(receiver, amount=-100)
        
    # bad case - transact not enough money
    def test_transaction_greater_than_balance(self, sender, receiver):
        with pytest.raises(ValueError):
